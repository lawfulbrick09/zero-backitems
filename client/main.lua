...
getweaponthing = function(weapon)
	for k,v in pairs(components_data) do 
		if v.model == GetHashKey(weapon) then
			return v
		end
	end
end
getweaponthing2 = function(weapon)
	for k,v in pairs(components_data) do 
...
isitemintable = function (item,table)
...
GetWeaponComponents = function(weapon)
	...
	defualt = false
	local weaponstable = {}
	if Config[weapon:upper()].defaultcomps then
		hascomponents = true
		defualt = true
		for k,v in pairs(Config[weapon:upper()].defaultcomps) do
			weapontable = getweaponthing(v)
			components[weapontable.model] = weapontable
			weaponstable[weapontable.model] = weapontable
		end
	end
    if data then
						...
						for k,v4 in pairs(compodata) do
							if v4.item == v3 then
								...
								hascomponents = true
									...
									local item = getweaponthing2(v4.name)
									local bonelist = {}
									for k,v in pairs(components) do
										if not isitemintable(v.bone,bonelist) then
											table.insert(bonelist,item.bone)
										end
									end
									for k,v in pairs(weaponstable) do 
										--print(item.bone.." "..v.bone.." "..tostring(item.bone==v.bone))
										if item.bone == v.bone then
											weaponstable[k] = v4
											components[k] = v4
										elseif not isitemintable(v.bone,bonelist) then
											weaponstable[v3] = v4
											components[v3] = v4
										end
									end 
...
RegisterNetEvent('QBCore:Server:RemoveItem', function(name,count)
...
RegisterNetEvent('QBCore:Server:AddItem', function(name,count)
	local exist = false
	for k,v in pairs(items) do
		if name == v.name then
			exist = true
